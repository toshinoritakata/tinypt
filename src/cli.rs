//! CLI（`tinypt`）とビューア（`viewer`）が共有するコマンドライン解析。
//!
//! 引数の解析・使い方の文面・シーン読み込み（CLI 上書きの優先順位解決）をここに置き、
//! 2 つのバイナリが同じ挙動になるようにする。

use crate::{build_default_scene, load_scene, RenderConfig, Tonemap};
...
impl CliOverrides {
    ...
    /// 呼び出し側が埋める。
    pub fn resolution(&self) -> (Option<usize>, Option<usize>) {
...
pub fn parse_args(args: impl IntoIterator<Item = String>, config: &mut RenderConfig) -> (CliOverrides, Vec<String>) {
...
/// シーンを構築する（`--scene` があれば Mitsuba XML、無ければ組み込みシーン）。
///
/// シーンファイルの設定より CLI 明示値を優先する唯一の解決ポイント。
/// CLI の解像度はローダーに渡す: センサーのアスペクト比が解像度から決まるので、
/// 読み込んだ後に width/height だけ差し替えると画角がずれる。
pub fn load_with_overrides(config: &mut RenderConfig, overrides: &CliOverrides) -> std::io::Result<Scene> {
    let forced_resolution = overrides.resolution();
    let scene = if let Some(path) = config.scene_path.clone() {
        load_scene(&path, config, forced_resolution)?
    } else {
