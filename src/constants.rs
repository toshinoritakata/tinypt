...
/// 自己交差回避オフセットの、シーンの大きさ（境界ボックスの対角線長）に対する比。
/// 実際のオフセットは [`World::ray_epsilon`](crate::world::World::ray_epsilon) が
/// `RAY_EPSILON_REL × 対角線長` として求める。シーン全体を拡大縮小しても、オフセットが
/// 同じ比率で変わるので結果が変わらない（スケール不変）。f64 の丸め誤差（相対 ~1e-16）より
/// 十分大きく、形状の細部（接地部の影など）より十分小さい値にしている。
pub const RAY_EPSILON_REL: f64 = 1e-7;

/// 形状のない（境界ボックスが空の）シーンで使う自己交差回避オフセット。
pub const RAY_EPSILON_EMPTY_SCENE: f64 = 1e-4;
...
pub const RENDER_REVISION: u32 = 8;
