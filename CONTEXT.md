...
**Ray epsilon** (`World::ray_epsilon`, `Hit.ray_eps`):
The self-intersection offset used whenever a ray leaves a surface: scattered rays start at `hit.p + ray_eps·d` (inside the BSDF, from `Hit.ray_eps`), shadow rays start at `p + ray_eps·wi` with `tmax = dist·(1 − 1e-9) − 2·ray_eps`, and all such rays use `ray_eps` as `tmin` (instances map it into object space). It is `RAY_EPSILON_REL (1e-7) ×` the diagonal of the scene's bounding box, so scaling a whole scene scales the offset with it and the image does not change. Its known limit is a scene that mixes very large and very small geometry (a huge floor with tiny objects): the offset is then too large for the small objects and their contact shadows and thin occluders are lost. A per-hit offset from floating-point error bounds is planned to remove that limit.
_Avoid_: ray epsilon as a fixed absolute distance.
