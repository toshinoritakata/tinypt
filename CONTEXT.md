...
Triangles use PBRT v4's watertight intersection (translate to the ray origin, permute so the dominant ray axis is z, shear, 2D edge functions where exactly-zero edges count as inside), so rays through an edge or vertex shared by adjacent triangles never slip through (the earlier Möller–Trumbore test missed about 2% of rays aimed exactly at a shared edge). `World::hit` rejects an instance with a conservative world-space bounding box (transformed mesh bounds padded by the transform's error bound) before transforming the ray into object space.
